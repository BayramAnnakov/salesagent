...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    response = agent.chat("Search the upcoming sales calendar events on March 17th 2024. Format the response for telegram message.")
    await context.bot.send_message(chat_id=update.effective_chat.id, text=str(response), parse_mode="Markdown")

    response = agent.chat("Prepare a memo how to prepare for this private jet services sales call using info about the event participant from their LinkedIn profile. Score this lead's success probability from 1 to 10 based on the LinkedIn profile information and the upcoming sales call event details. List possible topics or questions to discuss/ask to make the sales call successful. Format the response for telegram message.")
    await context.bot.send_message(chat_id=update.effective_chat.id, text=str(response), parse_mode="Markdown")

    response = agent.chat("Analyze the sales call using meeting transcript that is downloaded by zoom meeting id. Score the sales call from 1 to 10. Format response for doc file")
